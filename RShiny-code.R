...
reactive({
  ...
  list(
       ...
       # s2 probs
       s2.p3=input$s2.p3,
       s2.p4=input$s2.p3, # same as prob s2.p3
       # s3 probs
...
infections.conf<-
  data.frame(s1.infections.conf= paste0(sprintf("%.f",b$s1.infections),
                              " (",
                              sprintf("%.f",b$s1.infections.low),
                              " to ",
                              sprintf("%.f",b$s1.infections.high),
                              ")"),
         s2.infections.conf= paste0(sprintf("%.f",b$s2.infections),
                              " (",
                              sprintf("%.f",b$s2.infections.low),
                              " to ",
                              sprintf("%.f",b$s2.infections.high),
                              ")"),
         s3.infections.conf= paste0(sprintf("%.f",b$s3.infections),
                              " (",
                              sprintf("%.f",b$s3.infections.low),
                              " to ",
                              sprintf("%.f",b$s3.infections.high),
                              ")")) 
...
cost.conf<-
  data.frame(s1.cost.conf= 0,
         s2.cost.conf= paste0(sprintf("%.f",a$s2.total.cost),
                              " (",
                              sprintf("%.f",a$s2.total.cost.low),
                              " to ",
                              sprintf("%.f",a$s2.total.cost.high),
                              ")"),
         s3.cost.conf= paste0(sprintf("%.f",a$s3.total.cost),
                              " (",
                              sprintf("%.f",a$s3.total.cost.low),
                              " to ",
                              sprintf("%.f",a$s3.total.cost.high),
                              ")")) 
...
summary.table$ICER[2]<-sprintf("%.f",
                        summary.table$cost[2]/
        (summary.table$infections[1]-
           summary.table$infections[2]))
  summary.table$ICER[3]<-sprintf("%.f",
...
summary.table<-summary.table %>%
  mutate(Strategy=strategy,
         Infections=paste(sprintf("%.f",infections),
                          infections.conf),
         Cost=paste(sprintf("%.f",cost),
                    cost.conf)) %>%
  select(Strategy,
         Infections,
         Cost,
...
data.frame(Strategy="S1",
          p.ce=paste0(sprintf("%.f",working.probability.cost.effective$s1*100),
          "%")
          ),
data.frame(Strategy="S2",
          p.ce=paste0(sprintf("%.f",working.probability.cost.effective$s2*100),
                      "%")
          ),
data.frame(Strategy="S3",
          p.ce=paste0(sprintf("%.f",working.probability.cost.effective$s3*100),
              "%")
